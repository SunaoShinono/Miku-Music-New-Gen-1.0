            ...
            try {
                ...
                if (deleteLog) {
                    ...
                    if (executorMember && !executorMember.permissions.has(PermissionsBitField.Flags.Administrator)) {
                        await executorMember.ban({ reason: 'ระบบ Anti-Nuke ได้พบเจอการกระทำที่น่าสงสัยของท่าน เรื่อง ลบหลายช่องแชทพร้อมกัน' });

                       
                        const owner = await channel.guild.fetchOwner();
                        if (owner) {
                            await owner.send(`ผู้ใช้ ${executor.tag} ได้ถูกแบนแล้ว เนื่องจากระเบิดช่องแชทมากมายพร้อมกัน`);
                        } else {
                        ...
                        const embed = new EmbedBuilder()
                            .setColor('#ff0000')
                            .setTitle('Anti-Nuke: ลบช่องข้อความ')
                            .setDescription(`ผู้ใช้ ${executor.tag} ได้ถูกแบนเรียบร้อย เนื่องจาก ลบช่องข้อความหลายช่อง`)
                            .addFields(
                                { name: 'ผู้ใช้', value: `${executor.tag} (${executor.id})`, inline: true },
                                { name: 'ช่องข้อความที่ถูกลบ', value: `${nukeData.deletions}`, inline: true },
                                { name: 'ลบไปแล้วเมื่อ', value: new Date(currentTime).toLocaleString(), inline: false }
                            )
                            .setTimestamp();
